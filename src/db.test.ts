...
describe('persistence',()=>{
  ...
  it('returns persisted data without injecting campaign-specific records',async()=>{const data=seedData();mocks.get.mockResolvedValue(data);const loaded=await loadData();expect(loaded).toEqual(data);expect(loaded.campaign).toEqual([]);expect(mocks.get).toHaveBeenCalledWith('state','app')});
